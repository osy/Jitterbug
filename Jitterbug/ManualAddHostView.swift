...
struct ManualAddHostView: View {
    ...
    private func addHost(hostString: String) {
        main.backgroundTask(message: NSLocalizedString("Resolving...", comment: "ManualAddHostView")) {
            ...
            if let address = addresses?[0] as? Data {
                DispatchQueue.main.async {
                    main.saveManualHost(identifier: hostString, address: address)
                }
            } else {
                throw NSLocalizedString("Failed to resolve host.", comment: "ManualAddHostView")
            }
        }
    }
}

struct ManualAddHostView_Previews: PreviewProvider {
